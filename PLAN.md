...
```bash
gittchi init          # 최초 설정 (이름, 펫 선택, GitHub 연동) + 현재 레포 hook 자동 설치
gittchi attach        # 새 프로젝트에 hook 추가 (init 이후 다른 레포에서)
...
프로젝트/.git/hooks/
└── post-commit       # gittchi hook (자동 설치)
```
